 ...
 * Tests the state management logic extracted from OrdersPage store.
 */
import { describe, expect, it, vi, beforeEach } from 'vitest';

// ── Mirrors the selectedOrderId lifecycle logic from OrdersPage ─────────────
...
describe('A2: draft autosave contract', () => {
  const DRAFT_KEY_PREFIX = 'kort_new_order_draft_';
  ...
  it('draft key is user-scoped', () => {
    expect(draftKey('user-1')).toBe('kort_new_order_draft_user-1');
    expect(draftKey('user-2')).toBe('kort_new_order_draft_user-2');
