...
function viewStorageKey(userId?: string) { return `kort_orders_view_${userId ?? 'guest'}`; }
function groupStorageKey(userId?: string) { return `kort_orders_grouped_${userId ?? 'guest'}`; }
...
type DisplayGroup =
  ...
  | { kind: 'batch'; orders: Order[] };

function itemSignature(item: Order['items'][number]) {
  return [
...
function formatOrderItemLabel(orderNumber: string, item: Order['items'][number]) {
  return `#${formatOrderItemNumber(orderNumber, item.position)} ${buildItemLine(item) || item.productName}`;
}

function groupSignature(order: Order): string {
...
function buildGroups(orders: Order[]): DisplayGroup[] {
...
export default function OrdersPage() {
  ...
  const { selectedOrderId, setSelectedOrderId, orderFilters, setOrderFilters, resetOrderFilters } = useOperationsUiStore();
  ...
  const { isAbsolute, can } = useEmployeePermissions();
    ...
    <div className={styles.root}>
        ...
        <div className={styles.toolbarRight}>
          ...
          {showToolbarCreateButton && (
            <button
              ...
              onClick={() => navigate(activeTab === 'wholesale' ? '/sales/new?type=wholesale' : '/sales/new')}
            >
      ...
      {!isLoading && !isError && orders.length === 0 && (
        <div className={styles.emptyState}>
          ...
          {!hasActiveFilters && (
            <button className={styles.emptyAction} onClick={() => navigate('/sales/new')}>+ Создать заказ</button>
          )}
        </div>
      )}
...
const OrderCard = memo(function OrderCard({ order, onSelectOrder, hasAlert, stockMap, warehouseState }: { order: Order; onSelectOrder: (id: string) => void; hasAlert?: boolean; stockMap?: VariantAvailabilityMap; warehouseState?: OrderWarehouseState }) {
...
const BatchCard = memo(function BatchCard({ group, onSelectOrder }: { group: { orders: Order[] }; onSelectOrder: (id: string) => void }) {
...
const OrderRow = memo(function OrderRow({ order, onSelectOrder, hasAlert, stockMap, warehouseState }: { order: Order; onSelectOrder: (id: string) => void; hasAlert?: boolean; stockMap?: VariantAvailabilityMap; warehouseState?: OrderWarehouseState }) {
  const overdue = isOverdue(order.dueDate);
  const first = order.items?.[0];
  const more = (order.items?.length ?? 0) - 1;
  const isNewOrConfirmed = order.status === 'new' || order.status === 'confirmed';
...
const BatchRow = memo(function BatchRow({ group, onSelectOrder }: { group: { orders: Order[] }; onSelectOrder: (id: string) => void }) {
  const [expanded, setExpanded] = useState(false);
  const { orders } = group;
  const first = orders[0];
  const item = first.items?.[0];
