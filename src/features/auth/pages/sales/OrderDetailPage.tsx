...
export default function OrderDetailPage() {
  ...
  const setSelectedOrderId = useOperationsUiStore((state) => state.setSelectedOrderId);
  ...
  const totalDue = calculateOrderFinancials({
  ...
  const balance = getOrderBalance(totalDue, order.paidAmount);
          ...
          <div className={styles.card}>
            ...
            <div className={styles.actions}>
              {!['completed', 'cancelled'].includes(order.status) && (
                <button className={`${styles.actionBtn} ${styles.actionEdit}`} onClick={() => navigate(`/sales/${order.id}/edit`)}>
                  <Pencil size={13} />
                  Редактировать заказ
                </button>
              )}
