...
import modalStyles from '../../documents/InvoicePreviewModal.module.css';
...
export default function OrderDetailModal({ orderId, open, onClose }: Props) {
  ...
  const { canShipWithoutPayment } = useOperationsAccess();
                ...
                <div className={modalStyles.orderCard} style={{ pointerEvents: 'none', cursor: 'default' }}>
                  ...
                  {paidAmount < totalAmount && (
                    <div style={{ fontSize: 12, color: 'var(--fill-negative)' }}>
                      Остаток: {fmtMoney(getOrderBalance(totalAmount, paidAmount))}
                    </div>
                  )}
                </div>
                    ...
                    {paidAmount < totalAmount && (
                      <div style={{ fontSize: 13, color: 'var(--text-secondary)', gridColumn: '1 / -1' }}>
                        ...
                        <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--fill-negative)' }}>
                          {fmtMoney(getOrderBalance(totalAmount, paidAmount))}
                        </div>
                      </div>
                    )}
            ...
            {showReturnForm ? (
                ...
                {closeUnpaidWarning && (
                  <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
                      ...
                      {paymentStatus !== 'paid'
                        ? <strong style={{ color: 'var(--fill-negative)' }}>Заказ не оплачен — остаток: {fmtMoney(getOrderBalance(totalAmount, paidAmount))}</strong>
                ...
                {canShipWithoutPayment && paymentStatus !== 'paid' && (
                  <div style={{
                    ...
                    <span style={{ color: 'var(--fill-warning)', fontWeight: 600 }}>
                      Заказ не оплачен — остаток: {fmtMoney(getOrderBalance(totalAmount, paidAmount))}. Отправка производится вручную.
                    </span>
                  </div>
                )}
                {showShipForm ? (
              ...
              <div style={{ display: 'flex', gap: 10, width: '100%', alignItems: 'stretch' }}>
                <div style={{
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text-secondary)', paddingLeft: 23 }}>
                    Остаток: {fmtMoney(getOrderBalance(totalAmount, paidAmount))} — свяжитесь с менеджером
                  </div>
                </div>
                {status === 'on_warehouse' && (
                  <button
