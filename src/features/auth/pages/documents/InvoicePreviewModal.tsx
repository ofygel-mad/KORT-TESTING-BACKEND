...
function getHistoryKey(invoiceId: string) { return `kort_invoice_edits_${invoiceId}`; }
...
export default function InvoicePreviewModal({
  ...
  const { canEditInvoicePrices } = useOperationsAccess();
    ...
    if (invoiceMode && invoice) {
      ...
      if (prevSnapshot) {
        ...
        if (changes.length > 0) {
          ...
          void apiClient.post(`/invoices/${invoice.id}/edit-history`, entry).catch(() => {});
        }
      }
