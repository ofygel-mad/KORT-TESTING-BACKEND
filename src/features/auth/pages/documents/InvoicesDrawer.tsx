...
import type { Invoice, InvoiceStatus } from '@/entities/order/types';
import { useOperationsAccess } from '@/shared/hooks/useOperationsAccess';
import { useAuthStore } from '@/shared/stores/auth';
import InvoicePreviewModal from './InvoicePreviewModal';
import styles from './InvoicesDrawer.module.css';
...
export type FilterTab = 'all' | InvoiceStatus;
...
export default function InvoicesDrawer({ open, onClose, initialFilter = 'all' }: Props) {
  const { data, isLoading } = useInvoices({ limit: 100 });
  const allInvoices: Invoice[] = data?.results ?? [];
  ...
  const { canConfirmInvoice, canAccessWarehouseNav } = useOperationsAccess();
