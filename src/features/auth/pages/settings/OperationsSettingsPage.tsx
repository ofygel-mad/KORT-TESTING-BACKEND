...
import { useOperationsCatalogs, useOperationsSettings, useSaveCatalogs, useSaveProfile, useSettingsClients, useChangeEmail } from '@/entities/order/queries';
import { AlertCircle, Plus, RefreshCw, Save, X } from 'lucide-react';
import type { OperationsCatalogs } from '@/entities/order/types';
import {
  buildSizeCatalog,
...
import styles from './OperationsSettingsPage.module.css';
...
function emptyDraft(): OperationsCatalogs {
  return { productCatalog: [], sizeCatalog: [], workers: [], paymentMethodCatalog: [] };
}

function CatalogsTab() {
  const { data: catalogs, isLoading } = useOperationsCatalogs();
  const saveCatalogs = useSaveCatalogs();

  const [draft, setDraft] = useState<OperationsCatalogs | null>(null);
...
function ProfileTab() {
  const { data: profile, isLoading } = useOperationsSettings();
...
function ClientsTab() {
  const { data, isLoading } = useSettingsClients();
