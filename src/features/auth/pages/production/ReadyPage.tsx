...
import type { Order, InvoiceDocumentPayload, OrderStatus, Priority, Urgency } from '@/entities/order/types';
...
function viewStorageKey(userId?: string) {
  return `kort_ready_view_${userId ?? 'guest'}`;
}

function groupStorageKey(userId?: string) {
  return `kort_ready_grouped_${userId ?? 'guest'}`;
}
...
function hasWarehouseFulfillment(order: Order): boolean {
...
function buildItemSignature(orderItem: Order['items'][number]) {
  return [
...
function groupSignature(order: Order) {
  return [
...
export default function ReadyPage() {
  ...
  const openInvoicesDrawer = useOperationsUiStore((s) => s.openInvoicesDrawer);
  ...
  async function handleBatchInvoiceDownload() {
    ...
    try {
      const response = await apiClient.post('/orders/batch-invoice', {
  ...
  return (
    ...
    >
      ...
      <div className={`${styles.readyCell} ${styles.cellAmount}`}>
        <span className={styles.amount}>{formatMoney(calculateOrderFinancials({
    ...
    >
      ...
      <div className={styles.rowMain}>
        ...
        <div className={styles.rowMeta}>
          ...
          <span>{formatMoney(calculateOrderFinancials({
            itemsSubtotal: order.totalAmount,
            orderDiscount: order.orderDiscount,
            deliveryFee: order.deliveryFee,
            bankCommissionPercent: order.bankCommissionPercent,
