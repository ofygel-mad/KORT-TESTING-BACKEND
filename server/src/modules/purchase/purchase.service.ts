...
async function nextInvoiceNumber(orgId: string): Promise<string> {
  const last = await prisma.manualInvoice.findFirst({
    where: { orgId },
...
export async function create(
  ...
  return prisma.manualInvoice.create({
    data: {
      orgId,
      invoiceNum,
      type: dto.type,
...
export async function update(
  ...
  return prisma.$transaction(async (tx) => {
    if (dto.items !== undefined) {
      await tx.manualInvoiceItem.deleteMany({ where: { invoiceId: id } });
      await tx.manualInvoiceItem.createMany({
    ...
    return tx.manualInvoice.update({
      where: { id },
      data: {
...
export async function archive(orgId: string, id: string) {
  ...
  return prisma.manualInvoice.update({
    where: { id },
