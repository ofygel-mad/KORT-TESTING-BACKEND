...
import { validateStatusTransitionRules } from '../orders/status-validator.js';
import {
...
function isMissingInvoiceSchemaError(error: unknown) {
  ...
  if (error.code === 'P2021') {
    ...
    return table.includes('invoices') || table.includes('invoice_orders');
  }
    ...
    for (let attempt = 0; attempt < 5; attempt += 1) {
      try {
        return await prisma.$transaction(async (tx) => {
          const existingPending = await tx.invoiceOrder.findMany({
            where: {
              orderId: { in: orderIds },
