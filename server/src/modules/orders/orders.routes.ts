...
import { generateInvoiceXlsx, generateBatchInvoiceXlsx } from '../invoices/invoice.service.js';
import type { InvoiceDocumentPayload } from '../invoices/invoice-document.js';
...
export async function ordersRoutes(app: FastifyInstance) {
  ...
  // POST /api/v1/orders/:id/fulfill-from-stock
  ...
  // POST /api/v1/orders/:id/route-items
  ...
  // POST /api/v1/orders/:id/transfer/confirm
  ...
  // GET /api/v1/orders/:id/invoice?style=branded|default
  ...
  // POST /api/v1/orders/batch-invoice
  ...
  // POST /api/v1/orders/:id/activities
  // POST /api/v1/orders/:id/ship — Warehouse ships to client
  ...
  // PATCH /api/v1/orders/:id/requires-invoice — toggle invoice requirement
  ...
  // POST /api/v1/orders/:id/return-to-ready — warehouse returns order to "Готово"
  ...
  // POST /api/v1/orders/:id/items/:itemId/route — route single item immediately
  ...
  // POST /api/v1/orders/:id/change-request — manager submits item change request for in_production order
  ...
  // GET /api/v1/orders/change-requests — list pending change requests (for production page)
  app.get('/change-requests', async (request) => {
    return svc.listPendingChangeRequests(request.orgId);
  });

  // POST /api/v1/orders/change-requests/:crId/approve — seamstress approves
  ...
  // POST /api/v1/orders/change-requests/:crId/reject — seamstress rejects with reason
  ...
  // POST /api/v1/orders/:id/trash  — manager sends to trash
  app.post('/:id/trash', {
    preHandler: [app.authenticate, app.resolveOrg, app.requirePermission('orders.write', 'orders.admin')],
  }, async (request, reply) => {
    const { id } = request.params as { id: string };
  ...
  // POST /api/v1/orders/:id/restore-from-trash  — owner/full_access restores
  app.post('/:id/restore-from-trash', {
    preHandler: [app.authenticate, app.resolveOrg, app.requirePermission('orders.admin')],
  }, async (request, reply) => {
    const { id } = request.params as { id: string };
  ...
  // DELETE /api/v1/orders/:id  — permanent delete, owner/full_access only
  app.delete('/:id', {
    preHandler: [app.authenticate, app.resolveOrg, app.requirePermission('orders.admin')],
  }, async (request, reply) => {
    const { id } = request.params as { id: string };
  ...
  // GET /api/v1/orders/trash  — list trashed orders, owner/full_access only
  app.get('/trash', {
    preHandler: [app.authenticate, app.resolveOrg, app.requirePermission('orders.admin')],
  }, async (request) => {
  ...
  // GET /api/v1/orders/managers — list all active org members (for reassign dropdown)
  app.get('/managers', async (request) => {
    return svc.listOrgManagers(request.orgId);
  });

  // PATCH /api/v1/orders/:id/manager — reassign order to another manager
  // Access: owner | company.admin | employee with orders.admin
