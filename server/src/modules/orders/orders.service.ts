...
import { calculateOrderFinancials } from './financials.js';
...
async function nextOrderNumber(orgId: string, tx: Prisma.TransactionClient): Promise<string> {
  const rows = await tx.$queryRaw<Array<{ order_counter: number }>>`
  ...
  `;
  const row = rows[0];
  if (!row) throw new Error(`Organization not found for org ${orgId}`);
  return `${ORDER_NUMBER_PREFIX}-${String(row.order_counter).padStart(3, '0')}`;
}
...
async function resolveOrderClient(
  ...
  const createdClient = await tx.customer.create({
    data: {
      orgId,
      fullName: clientName,
      phone: lookupPhone,
...
export async function setRequiresInvoice(
  ...
  if (!order) throw new NotFoundError('Order', id);
  await prisma.order.update({ where: { id }, data: { requiresInvoice } });
  return { ok: true };
}
...
export async function addPayment(orgId: string, orderId: string, authorId: string, authorName: string, data: {
  ...
  const order = await prisma.order.findFirst({ where: { id: orderId, orgId } });
  ...
  const payment = await prisma.$transaction(async (tx) => {
    ...
    if (newPaymentStatus === 'paid') {
      await tx.unpaidAlert.updateMany({
...
export async function initiateTransfer(orgId: string, orderId: string) {
  ...
  return prisma.orderTransfer.create({
    data: { orderId },
  });
}
...
export async function confirmTransfer(orgId: string, orderId: string, by: 'manager' | 'client', authorId: string, authorName: string) {
  ...
  if (!order?.transfer) throw new NotFoundError('OrderTransfer');

  const updateData: Record<string, unknown> = {};
  if (by === 'manager') updateData.confirmedByManager = true;
  if (by === 'client') updateData.confirmedByClient = true;

  const updated = await prisma.orderTransfer.update({
...
export async function update(orgId: string, id: string, authorId: string, authorName: string, data: UpdateOrderInput) {
  const order = await prisma.order.findFirst({ where: { id, orgId }, include: { items: true } });
  if (!order) throw new NotFoundError('Order', id);
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (data.items) {
      ...
      if (order.status === 'confirmed') {
        await tx.productionTask.deleteMany({ where: { orderId: id } });
...
export async function restore(orgId: string, id: string, authorId: string, authorName: string) {
  ...
  await prisma.$transaction(async (tx) => {
    const restoreData: Prisma.OrderUpdateInput = {
      isArchived: false,
      archivedAt: null,
      completedAt: null,
...
export async function approveChangeRequest(
  ...
  if (!changeRequest) throw new NotFoundError('ChangeRequest', changeRequestId);
...
export async function routeSingleItem(
  ...
  if (!item) throw new NotFoundError('OrderItem', itemId);
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (fulfillmentMode === 'production') {
      ...
      await tx.productionTask.deleteMany({ where: { orderItemId: itemId } });
    }
 ...
 * Returns minimal shape: id + name (+ isOwner for display hint).
 */
